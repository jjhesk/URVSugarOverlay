...
public abstract class singleDetailPost<Frag> extends AppCompatActivity implements internalChangeInFragment<Frag> {
    ...
    private void initMainContentFragment(Frag mfragment, Bundle savestate) {
        if (savestate == null) {
            ...
            currentFragmentNow = (Frag) this.getFragmentManager().findFragmentById(R.id.lylib_main_frame_body);
        }
    }
    ...
    private void initToolBar(final @LayoutRes int resId) {
        try {
            if (SimpleLayout.BODY_LAYOUT.isToolbarOn(resId) || forceConfigureToolBar()) {
    ...
    public void setFragment(Frag fragment, String title, Frag oldFragment, boolean closeDrawer) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
            ...
            ft.replace(R.id.lylib_main_frame_body, (android.support.v4.app.Fragment) fragment).commit();
            ...
            ft.replace(R.id.lylib_main_frame_body, (android.app.Fragment) fragment).commit();
